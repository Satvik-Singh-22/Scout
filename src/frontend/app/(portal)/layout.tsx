...
export default function PortalLayout({
  ...
  const navItems = [
    ...
    ...(user?.role !== 'PLATFORM_ADMIN'
      ...
      : []),
    ...(user?.role !== 'PLATFORM_ADMIN'
      ? [{ href: '/scheduled', label: 'Scheduled', icon: 'schedule' }]
      : []),
    ...(user?.role !== 'PLATFORM_ADMIN'
      ? [{ href: '/team', label: 'Team', icon: 'groups' }]
      : []),
    ...(user?.role === 'DATA_OWNER'
      ? [{ href: '/onboarding', label: 'Data Config', icon: 'database' }]
      : []),
    ...(user?.role === 'PLATFORM_ADMIN'
        ...
        <aside className="bg-gray-50 dark:bg-gray-950 w-64 fixed left-0 top-16 bottom-0 hidden md:flex flex-col p-4 gap-2 border-r border-outline-variant/10 z-30 font-inter text-sm font-medium">
          ...
          <div className="mt-auto flex flex-col gap-2 pt-6 border-t border-gray-200/60">
            ...
            <button
              ...
              className="flex items-center gap-3 px-3 py-2.5 text-error hover:bg-red-50 rounded-lg text-sm font-medium transition-all w-full text-left"
            >
